...
public class AssembleService {
    ...
    public Result findAssemblesInTopic(String domainName, String topicName) {
        String res = jedis.get(RedisName.DOMAINID_TOPICID_ASSEMBLE + "::" + domainName + "_" + topicName);
        if (res != null && !res.isEmpty()) {
            List<Assemble> assembles = (List<Assemble>) JSON.parseArray(res, Assemble.class);
        ...
        String jsonAssembles = JSON.toJSONString(assembles);
        jedis.set(RedisName.DOMAINID_TOPICID_ASSEMBLE + "::" + domainId + "_" + topicId, jsonAssembles,"NX", "EX", 3600*24*1);
        return ResultUtil.success(ResultEnum.SUCCESS.getCode(), ResultEnum.SUCCESS.getMsg(), assembles);
    }
